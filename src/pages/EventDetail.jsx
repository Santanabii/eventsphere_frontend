...
export default function EventDetail() {
  ...
  if (!event) return (
    <div className="min-h-screen bg-[#0A0A18] flex items-center justify-center">
      <p className="text-[#8888AA]">Event not found</p>
    </div>
  )
    ...
    <div className="min-h-screen bg-[#0A0A18]">
      ...
      <div className="relative h-72 md:h-80 mt-20">
        {event.banner_image ? (
          ...
          <div className="w-full h-full bg-[#16163A]" />
        )}
        <div className="absolute inset-0 bg-gradient-to-t from-[#0A0A18] via-[#0A0A18]/40 to-transparent" />
        <button
          onClick={() => navigate(-1)}
          className="absolute top-6 left-6 flex items-center gap-2 glass px-4 py-2 rounded-full text-sm text-[#F0F0FF] hover:border-[#7C3AED] transition-colors"
        >
        ...
        <div className="grid lg:grid-cols-3 gap-10">
          ...
          <div className="lg:col-span-2">
            <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}>
              ...
              <h1 className="font-display font-bold text-3xl md:text-4xl text-[#F0F0FF] mb-6">{event.title}</h1>

              <div className="space-y-4 mb-8">
                <div className="flex items-center gap-3 text-[#8888AA]">
                  <div className="w-9 h-9 rounded-lg bg-[#16163A] border border-[#2A2A5A] flex items-center justify-center flex-shrink-0">
                    <Calendar size={16} />
                  </div>
                  <div>
                    <p className="text-[#F0F0FF] font-medium">{formatDate(event.date)}</p>
                ...
                <div className="flex items-center gap-3 text-[#8888AA]">
                  ...
                  <div>
                    <p className="text-[#F0F0FF] font-medium">{event.venue}</p>
                ...
                <div className="flex items-center gap-3 text-[#8888AA]">
                  ...
                  <div>
                    <p className="text-[#F0F0FF] font-medium">Organised by {event.organiser_name}</p>
              ...
              <div className="card p-7">
                <h3 className="font-display font-semibold text-lg text-[#F0F0FF] mb-3">About this event</h3>
                <p className="text-[#8888AA] leading-relaxed">{event.description}</p>
              </div>
            </motion.div>
          </div>
          ...
          <div className="lg:col-span-1">
            <motion.div initial={{ opacity: 0, x: 16 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.15 }}
              ...
              <h3 className="font-display font-semibold text-lg text-[#F0F0FF] mb-6 flex items-center gap-2">
                <Ticket size={18} className="text-[#A855F7]" /> Get tickets
              </h3>
                ...
                {event.tiers?.length > 0 ? event.tiers.map(tier => (
                  <button
                    ...
                    className={`w-full p-4 rounded-xl border text-left transition-colors ${
                      selectedTier?.id === tier.id ? 'border-[#7C3AED] bg-[#7C3AED]/10' : 'border-[#2A2A5A] hover:border-[#3D3D75]'
                    } ${!tier.is_available ? 'opacity-50 cursor-not-allowed' : ''}`}
                    disabled={!tier.is_available}
                  >
                    <div className="flex justify-between items-start">
                      <div>
                        <p className="font-medium text-[#F0F0FF]">{tier.name}</p>
                        <p className="text-xs text-[#6E6E96] mt-1">{tier.quantity_remaining} remaining</p>
                      </div>
                      <div className="text-right">
                        <p className="font-display font-semibold text-[#F0F0FF]">
                  ...
                  <p className="text-[#6E6E96] text-sm text-center py-4">No tickets available yet</p>
                )}
              </div>
              ...
              {polling ? (
                <div className="text-center py-4">
                  ...
                  <p className="text-sm text-[#8888AA]">Waiting for payment confirmation...</p>
                </div>
              ) : (
              ...
              {!user && (
                <p className="text-center text-[#6E6E96] text-xs mt-3">
                  <button onClick={() => navigate('/login')} className="text-[#A855F7] underline">Sign in</button>
                  {' '}to purchase tickets
                </p>
              )}
            </motion.div>
