...
export default function CreateEvent() {
  ...
  const [tiers, setTiers] = useState([{ name: '', price: '', quantity: '', sale_start: '', sale_end: '' }])
            ...
            <div className="card p-7 space-y-5">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. Afro Night Nairobi 2026" required className="input"
                />
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Tell people about your event..." required rows={4}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    id="venue" type="text" value={form.venue}
                    onChange={e => setForm({ ...form, venue: e.target.value })}
                    placeholder="e.g. KICC Nairobi" required className="input"
                  />
                </div>
                <div>
                  <label htmlFor="date" className="field-label">Date & time *</label>
                  <input
                    id="date" type="datetime-local" value={form.date}
            ...
            <div className="card p-7 space-y-4">
              ...
              {form.resale_allowed && (
                <div>
                  ...
                  <input
                    id="cap" type="number" value={form.resale_price_cap}
                ...
                <div key={i} className="bg-[#0A0A18] rounded-xl p-4 space-y-3 border border-[#2A2A5A]">
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    <input type="text" value={tier.name} onChange={e => updateTier(i, 'name', e.target.value)}
                      placeholder="e.g. VIP" aria-label={`Tier ${i + 1} name`} className="input py-2.5 text-sm" />
                    <input type="number" value={tier.price} onChange={e => updateTier(i, 'price', e.target.value)}
                      placeholder="Price (KES)" aria-label={`Tier ${i + 1} price`} className="input py-2.5 text-sm" />
                    <input type="number" value={tier.quantity} onChange={e => updateTier(i, 'quantity', e.target.value)}
                      placeholder="Quantity" aria-label={`Tier ${i + 1} quantity`} className="input py-2.5 text-sm" />
                  </div>
