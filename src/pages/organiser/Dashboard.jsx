...
export default function Dashboard() {
  ...
  const statusBadge = { published: 'badge-green', draft: 'badge-amber', closed: 'badge-muted' }
      ...
      <div className="max-w-6xl mx-auto">
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}
          ...
          <div>
            ...
            <h1 className="font-display font-bold text-3xl text-zinc-100">Welcome, {user?.username}</h1>
        ...
        <div className="grid grid-cols-3 gap-5 mb-10">
          {[
            ...
            <motion.div key={i} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.06 }} className="card p-6">
              <div className="w-9 h-9 rounded-lg bg-violet-500/10 text-violet-400 flex items-center justify-center mb-4">{stat.icon}</div>
              <p className="font-display font-bold text-2xl text-zinc-100">{stat.value}</p>
        ...
        <div>
          <h2 className="font-display font-semibold text-xl text-zinc-100 mb-6">Your events</h2>
          ...
          {isLoading ? (
            ...
            <div className="card p-12 text-center">
              ...
              <p className="text-lg font-medium text-zinc-100">No events yet</p>
              <p className="text-zinc-500 mt-1 mb-6">Create your first event to get started</p>
              ...
              {events.map((event, i) => (
                <motion.div key={event.id} initial={{ opacity: 0, x: -12 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.04 }}
                  ...
                  <div className="flex items-center gap-4 flex-1 min-w-0">
                    <div className="w-11 h-11 rounded-xl bg-zinc-900 border border-zinc-800 flex-shrink-0 overflow-hidden">
                      {event.banner_image ? (
                    ...
                    <div className="min-w-0">
                      <h3 className="font-medium text-zinc-100 truncate">{event.title}</h3>
                      <p className="text-sm text-zinc-400 truncate">{event.venue}</p>
                    </div>
                  </div>

                  <div className="flex items-center gap-3 flex-shrink-0">
                    <span className={`badge capitalize ${statusBadge[event.status] || 'badge-muted'}`}>{event.status}</span>
                    <span className="text-xs text-zinc-500">{event.tiers?.length || 0} tiers</span>
                    <Link to={`/organiser/analytics/${event.id}`}>
                      <button className="flex items-center gap-1.5 px-3 py-2 rounded-lg bg-violet-500/10 text-violet-400 hover:bg-violet-500/15 transition-colors text-sm">
                        <BarChart3 size={14} /> Analytics
                      </button>
                    </Link>
                    <Link to={`/events/${event.id}`}>
                      <button className="flex items-center gap-1.5 px-3 py-2 rounded-lg bg-zinc-900 border border-zinc-800 text-zinc-400 hover:text-zinc-100 transition-colors text-sm">
                        <Eye size={14} /> View
                      </button>
                    </Link>
                  </div>
