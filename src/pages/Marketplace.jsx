...
export default function Marketplace() {
  ...
  const handlePurchase = async () => {
    ...
    try {
      ...
      setBuyingListing(null); setPhone('')
    } catch (err) {
              ...
              return (
                <motion.div key={listing.id} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }}
                  ...
                  <div className="border-t border-[#2A2A5A] pt-4 flex items-center justify-between">
                    ...
                    <button
                      onClick={() => { if (!user) { navigate('/login'); return } setBuyingListing(listing) }}
      ...
      <AnimatePresence>
        {buyingListing && (
          <div
            ...
            role="dialog" aria-modal="true" aria-labelledby="buy-modal-title"
          ...
          >
            <motion.div initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.96 }}
              ...
              <div className="flex gap-3">
                <button onClick={() => { setBuyingListing(null); setPhone('') }} className="btn-ghost flex-1 py-3 text-sm">Cancel</button>
                <button onClick={handlePurchase} disabled={purchasing || !phone} className="btn-primary flex-1 py-3 text-sm">
